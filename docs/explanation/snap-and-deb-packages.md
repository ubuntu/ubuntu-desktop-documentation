...
The App Center on Ubuntu Desktop shows snaps by default, both in the search and the category pages. Snap apps are usually *sandboxed*. That means that you can limit which system resources they can access.
...
When you search for deb packages in the App Center, you get applications from the {external:ref}`official Ubuntu repositories <package-archive>`. You can check if an app is a deb package on its App Center page.
